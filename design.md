# H-Intelligence — AML/Compliance Portal
## CLAUDE.md — Design & Development Reference

> **Brand:** Harbour and Hills Financial Services Limited (H&H)
> **Product Name:** H-Intelligence
...
H-Intelligence is a professional AML (Anti-Money Laundering) and compliance screening web portal built for H&H's clients and internal administrators. It provides name screening, bulk screening, ongoing monitoring, case management, and reporting — comparable in depth to LSEG World-Check One, but with H&H's own branding, workflow logic, and subscription management layer.

The portal has two distinct front-ends sharing a common design system:

| Portal | Audience | Key Focus |
|---|---|---|
| **Customer Portal** | H&H's end-clients (compliance officers, AML analysts) | Screening, case management, reporting |
| **Admin Portal** | H&H internal admins | Masters, subscriptions, packages, user management |
...
- **Brand name in UI:** "H-Intelligence" (product), "H&H" or "Harbour and Hills" (company)
...
--color-primary-700: #023e63;   /* ← Main brand primary */
--color-primary-800: #012e4a;
--color-primary-900: #012236;
--color-primary-950: #001827;

/* Secondary (Teal) */
--color-secondary-50:  #eaf9f8;
--color-secondary-100: #d8f5f3;
--color-secondary-200: #b2ebe8;
--color-secondary-300: #7fdfd9;
--color-secondary-400: #4cd2c9;
--color-secondary-500: #00bfb3;  /* ← Main brand secondary / "brand" token */
--color-secondary-600: #00988f;
...
:root {
  ...
  /* ── Brand Colors ── */
  --primary:           #023e63;   /* Navy — primary CTA, active nav, links */
  --primary-foreground:#ffffff;   /* Text on primary bg */
  --brand:             #00bfb3;   /* Teal — accent, highlights, brand badge */
  ...
  --ring:              #00bfb3;   /* Focus ring — TEAL/brand (updated June 2026; was navy). Applied as a soft glow via --shadow-focus */

  /* ── Sidebar Specific ── */
  --sidebar-foreground:        #0a0a0a;
  --sidebar-primary:           #023e63;
  ...
  /* ── Charts ── */
  --chart-1:            #023e63;  /* Primary data series (navy) */
  --chart-2:            #00bfb3;  /* Secondary data series (teal) */
...
[data-theme="dark"] {
  ...
  --primary:           #023e63;   /* Navy stays the same */
...
> **Updated June 2026:** Sharpened the whole scale for a crisper, more enterprise look. Previous values were 6 / 8 / 12 / 16px (sm→xl) with 8 / 8 / 10 / 6px components.

```css
--radius-sm:   3px;   /* Checkboxes, small chips */
--radius-md:   4px;   /* Sidebar nav items */
...
--component-radius-button:   4px;
--component-radius-input:    4px;
--component-radius-badge:    4px;
--component-radius-checkbox: 3px;
```
...
box-shadow: 0 0 0 3px rgba(0, 191, 179, 0.25);
```
...
|---|---|---|---|---|
| Default (Primary) | `--primary` (#023e63) | `--primary-foreground` (#fff) | `--primary-800` (#012e4a) | Primary CTA |
...
| Brand | `--brand` (#00bfb3) | `--brand-foreground` (#fff) | `--secondary-600` | Brand highlights |
...
- Nav item (active): background `--sidebar-primary` (#023e63), color `--sidebar-primary-foreground` (#fff)
...
- Active state: filled primary color highlight on menu item
- Sub-items: indented, smaller font, toggled by parent click
- Top header: H-Intelligence logo (left), user avatar + name (right), notification bell, portal switcher if admin has dual access
...
- **Total Profiles Screened** — links to Case Manager (all cases in period)
- **Total Alert Disposed** *(previously "Total Names Filtered" — use this corrected label)*
- **Unresolved Hits** — count is 0 only when ALL hits in ALL cases are resolved
- **True Matches** — count of true match resolutions in period
- **False Matches** — count of false match resolutions in period
...
- Shows H&H-managed databases (OFAC, EU, UN, UK, etc.) and last update timestamp
- Most recently updated database shown at top (descending by update date)
- Auto-updates when H&H (Xini/backend team) refreshes the database
...
- Manage H&H internal admin users
...
- Dark theme shown in reference — H-Intelligence uses light theme but same information density
...
2. **PEP Database Selection:** When "PEP" is selected as a database filter, should the user be able to select which specific PEP database to query against? Same question for Sanctions and Adverse Media. **→ Confirm with Vilas/H&H.**
...
5. **Admin vs Customer Portal URL structure:** Single domain with path-based routing (`/admin/*` vs `/app/*`) or separate subdomains (`admin.h-intelligence.com` vs `app.h-intelligence.com`)? **→ Confirm with tech team.**

6. **World-Check Integration:** Are the screening results powered by LSEG World-Check API, or H&H's own database? Screenshots suggest World-Check reference data — clarify the data source architecture.
...
*Last updated: June 2026 | Prepared for H&H H-Intelligence Portal Development*
