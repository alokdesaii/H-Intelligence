/**
 * H-Intelligence Admin Shell Layout Injector
 * Adheres to DESIGN.md constraints:
 * - 256px rigid Navy (#023e63) sidebar
 * - 64px white top bar
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const generateMenuHtml = () => {
    return activeNavItems.map(item => {
      if (item.subItems) {
        ...
        return `
          <div class="space-y-1">
            <button onclick="toggleSubMenu('${subMenuId}')" class="w-full flex items-center justify-between px-3 py-2.5 rounded-md text-[#b3c5d0] hover:text-white hover:bg-[#063352] transition-colors group">
            ...
            <div id="${subMenuId}" class="${isChildActive ? '' : 'hidden'} pl-4 space-y-1 border-l border-[#0f4e75] ml-5 mt-1">
  ...
  const sidebarHtml = `
    <div class="w-64 bg-[#023e63] flex flex-col justify-between shrink-0 border-r border-[#e5e5e5] h-full text-[#b3c5d0]">
      ...
      <div class="border-t border-[#0f4e75] p-4 flex flex-col gap-3">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-full bg-[#0d4c72] border border-[#155d88] flex items-center justify-center text-white font-semibold text-sm">
            AD
          </div>
          <div class="overflow-hidden">
            <p class="text-white text-xs font-semibold truncate leading-none mb-1">Alok Desai</p>
            <p class="text-[10px] text-[#8ba2b1] truncate leading-none">alok.desai@hh.com</p>
          </div>
        </div>
        
        <!-- Safety Role Badge -->
        <div class="bg-[#0c4469] border border-[#0f5482] rounded-md py-2 px-3 text-center">
